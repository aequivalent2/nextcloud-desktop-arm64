...
$env:PATH = "$QtDir\bin;C:\Qt\Tools\CMake\bin;C:\Qt\Tools\Ninja;" + $env:PATH
...
& "$QtDir\bin\qt-cmake.bat" C:\build\qtkeychain-src `
  -G Ninja `
...
# KF6Archive
# vcpkg wurde bereits im Workflow gebootstrapped (C:\build\vcpkg)
...
# Compression-Libs via vorhandenes vcpkg nachinstallieren
...
& "$QtDir\bin\qt-cmake.bat" C:\build\karchive-src `
  -G Ninja `
  -DCMAKE_BUILD_TYPE=Release `
...
& "$QtDir\bin\qt-cmake.bat" C:\build\kdsingleapp-src `
  -G Ninja `
  -DCMAKE_BUILD_TYPE=Release `
